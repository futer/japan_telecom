...
shinyServer(function(input, output) {
  ...
  output$whichplot <- renderPlot({
    ...
    if(input$plotnumber == 'Plot3th')
    {
      ...
      plotGgplot <- ggplot(df.totalFor2014,
    ...
    if(input$plotnumber == 'Plot4th')
    {
      ...
      plotGgplot <- ggplot(df2.totalFor2014,
                           aes(x = "", y=TotalFor2014, fill = ServiceProviders)) +
        geom_bar(width=1, stat="identity") +
        coord_polar(theta = "y") +
        scale_x_discrete("") +
